...
if (apiKey && !apiKey.startsWith("gsk_")) {
  ...
  const generationConfig = {
    ...
    topP: 0.95,
    topK: 64,
    maxOutputTokens: 16384, // Increased to handle longer itineraries (9+ days)
    ...
    history: [
      {
        role: "user",
        parts: [
          { text: "Generate Travel Plan as per the given info ,Give me a Hotels options list with HotelName, Hotel address, Price, hotel image url, geo coordinates, rating, descriptions and  suggest itinerary with placeName, Place Details, Place Image Url, Geo Coordinates, ticket Pricing, rating, Time travel each of the location for 3 days with each day plan with best time to visit in JSON format." },
        ],
      },
      {
        role: "model",
  ...
  sendMessage: async (prompt) => {
    if (apiKey && apiKey.startsWith("gsk_")) {
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are an expert travel planner. You must respond ONLY with a raw JSON object matching the requested schema. Do not wrap it in markdown code blocks (such as ```json ... ```) or add any other text."
            },
            {
              role: "user",
              content: prompt
      ...
      if (!activeChatSession) {
        throw new Error("Gemini AI API key is not configured or invalid.");
      }
      return await activeChatSession.sendMessage(prompt);
    }
  }
};
...
export const getFriendlyErrorMessage = (error) => {
  ...
  // Groq specific or general errors
  if (lowerMessage.includes("groq") || lowerMessage.includes("gsk_")) {
    if (lowerMessage.includes("401") || lowerMessage.includes("unauthorized") || lowerMessage.includes("api key") || lowerMessage.includes("invalid")) {
    ...
    if (lowerMessage.includes("429") || lowerMessage.includes("rate limit")) {
      return "Groq API rate limit exceeded. Please wait a moment or check your limits.";
    }
  }

  // Billing / Dunning issues
  if (lowerMessage.includes("dunning") || lowerMessage.includes("billing") || lowerMessage.includes("403")) {
    return "API access is currently disabled due to billing or subscription status on the Google Cloud project. Please verify your billing details or use a different API key.";
  }

  // Quota / Rate limit issues
  if (lowerMessage.includes("quota") || lowerMessage.includes("429") || lowerMessage.includes("rate limit")) {
    return "API rate limit exceeded or quota limit reached. Please try again in a few moments.";
  }

  // API Key issues
  if (lowerMessage.includes("api key") || lowerMessage.includes("invalid key") || (lowerMessage.includes("400") && lowerMessage.includes("key"))) {
    return "Invalid Google Gemini API key. Please check your .env configuration.";
  }

  // Model issues
  if (lowerMessage.includes("model") && (lowerMessage.includes("not found") || lowerMessage.includes("404"))) {
    return "The requested Gemini model is not found or unsupported. Please check the model name configuration.";
  }
  ...
  return error.message || "Unable to generate trip plan. Please try again.";
};
